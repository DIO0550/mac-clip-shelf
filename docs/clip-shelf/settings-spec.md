...
| `ShortcutRecorder` | カスタム `View`（モーダル） | キャプチャ式の入力UI（MASShortcut を使用） |
...
| ショートカット | ピッカーを開く | `KeyCombination` | `⌘⇧V` | `shortcut.picker` |
| ショートカット | 履歴ブラウザを開く | `KeyCombination` | `⌘⇧H` | `shortcut.history` |
| 外観 | テーマ | `enum {system,light,dark}` | `.system` | `appearance` |

永続化先は SQLite の `settings_kv` テーブル（詳細: [history-table-spec.md](./history-table-spec.md)）。`UserDefaults` ではなく SQLite を使う理由は、テーブルベースで一元管理し、エクスポート/インポートを容易にするため。
...
- [clipboard-monitor-spec.md](./clipboard-monitor-spec.md) — ショートカット登録の挙動
- [history-table-spec.md](./history-table-spec.md) — `settings_kv` テーブル
