...
│   │   ├── SettingsStore.swift
│   │   └── ModelContainer+ClipShelf.swift   # SwiftData コンテナ生成
│   ├── UI/
│   │   ├── MenuBar/
...
- `Domain` は他のどこにも依存しない純粋な型のみ（ただし `@Model` を持つエンティティは SwiftData に依存）
- `Services` は `Domain` と OS API、永続化（SwiftData / UserDefaults）に依存する
...
final class AppDependencies {
    ...
    init() throws {
        self.modelContainer = try ModelContainer.clipShelf()
        self.settings = SettingsStore()                          // UserDefaults ベース
        self.history  = HistoryService(container: modelContainer)
...
sequenceDiagram
    ...
    Main->>Deps: init()
    Deps->>MC: ModelContainer.clipShelf()
    MC->>MC: SchemaMigrationPlan 適用
    Deps->>SS: init()
    SS->>SS: register defaults
    Deps->>HS: init(container)
    Deps->>Mon: init(...)
    Deps->>HK: init(...)
    Main->>UI: init(deps)
...
| イベント | 動作 |
|:--------|:-----|
| 起動 | ModelContainer 生成（マイグレーション含む） → サービス初期化 → メニューバー登録 → ホットキー登録 → ポーリング開始 |
...
| 終了 | ホットキー解放 → ポーリング停止 → 未保存の ModelContext を `save()` |

## エラー方針

| レベル | 例 | 扱い |
|:------|:---|:-----|
| 致命的 | ストア破損、SchemaMigrationPlan 失敗 | アラート表示後にアプリ終了 or 「リセット」を促す |
...
| カテゴリ | `app.clipboard`, `app.paste`, `app.store`, `app.ui` |
...
| 対象 | 方針 |
|:-----|:-----|
| `HistoryService` | CRUD と重複判定をインメモリ `ModelContainer`（`isStoredInMemoryOnly`）で UT |
| `ClipboardMonitor` | `changeCount` 増加 → `add` 呼び出しの統合テスト（モック NSPasteboard 不可なので最小限） |
| `PasteService` | `CGEvent` 呼び出しは UT しにくいので手動確認のみ |
| マイグレーション | `VersionedSchema` 各段の `MigrationStage` を XCTest で実行 |
